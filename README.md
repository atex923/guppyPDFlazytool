...
- 系統匣按鈕：視窗右下角固定顯示 `.` 按鈕；Windows 可隱藏到系統匣，一般最小化維持縮到工具列。
...
- `Guppy_PDFlazyTool_V1.1.9.pyw`: versioned standalone no-console source
- `Guppy_PDFlazyTool_V1.1.9.py`: versioned Python source
...
- `Guppy_PDFlazyTool_V1.1.9.py`: versioned Python source for V1.1.9
- `Guppy_PDFlazyTool_V1.1.9.pyw`: standalone no-console source for V1.1.9
...
Google Drive handoff copies are organized under `12.Codex/Guppy_PDFlazyTool_versions/`. Each version keeps its own folder, for example `V1.1.9/`, with the matching `.py` and `.pyw` files. The `12.Codex` root may also keep the latest launcher and source for quick access.
