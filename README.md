...
- 系統匣按鈕：Windows 標題列 `▼` 按鈕貼齊最小化、最大化、關閉三鍵左側；點擊可隱藏到系統匣，一般最小化維持縮到工具列。
...
- `Guppy_PDFlazyTool_V1.4.5.pyw`: versioned standalone no-console source
- `Guppy_PDFlazyTool_V1.4.5.py`: versioned Python source
...
- `Guppy_PDFlazyTool_V1.4.5.py`: versioned Python source for V1.4.5
- `Guppy_PDFlazyTool_V1.4.5.pyw`: standalone no-console source for V1.4.5
...
Google Drive handoff copies are organized under `12.Codex/Guppy_PDFlazyTool/versions/`. Each version keeps its own folder, for example `V1.4.5/`, with the matching `.py` and `.pyw` files. The `12.Codex/Guppy_PDFlazyTool/latest/` folder and `12.Codex` root may also keep the latest launcher and source for quick access.
