...
class PDFRenameTool:
    def __init__(self, root):
        ...
        self.minimize_to_tray_var = tk.BooleanVar(
            value=platform.system().lower() == "windows"
        )
        self.tray_icon = None
    ...
    def create_signature_footer(self):
        ...
        self.footer_controls = tk.Frame(footer, bg=BG)
        self.footer_controls.grid(
            row=0,
            column=2,
            sticky="se",
            padx=(8, 14),
        ...
        if platform.system().lower() == "windows":
            ...
            self.tray_checkbox.pack(side="left", padx=(0, 10))
    ...
    def windows_tray_enabled(self) -> bool:
        return (
            platform.system().lower() == "windows"
            and bool(self.minimize_to_tray_var.get())
        )

    def on_root_unmap(self, event=None):
        if event is not None and event.widget is not self.root:
            return
        if self._tray_hiding or not self.windows_tray_enabled():
            return
        ...
        self.root.after(80, check_minimized)
    ...
    def hide_to_system_tray(self):
        if not self.windows_tray_enabled():
            return
