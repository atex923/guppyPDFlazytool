    ...
    def recognize(self, img: Image.Image) -> str:
        ...
        if self.engine_name not in ("PaddleOCR", "RapidOCR", "EasyOCR"):
            return (
                "尚未載入 OCR 套件。\n"
                "請確認 OCR 檔案是否在同一資料夾內。\n\n"
                "可放在程式旁的 ocr_packages、external_packages 或 site-packages 資料夾。\n"
                "需要的套件：paddleocr paddlepaddle，或 rapidocr-onnxruntime，或 easyocr。"
            )
...
class PDFRenameTool:
    ...
    def create_ui(self):
        ...
        self.main.grid_columnconfigure(0, weight=1, minsize=360, uniform="maincols")
        self.main.grid_columnconfigure(1, weight=0, minsize=64)
        self.main.grid_columnconfigure(2, weight=1, minsize=360, uniform="maincols")
        self.main.grid_columnconfigure(3, weight=0, minsize=68)
    ...
    def create_preview_toolbar(self, parent):
        ...
        for text, cmd in (("‹", self.prev_page), ("›", self.next_page)):
            self.preview_button(left, text, cmd, 44).pack(side="left", padx=2)
