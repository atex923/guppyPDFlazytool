...
# V0.3.0  重新調整低解析度版面，修正 OCR 外部延遲載入與框選辨識流程。
# V0.3.1  加強低解析度 compact 排版，縮小字體、列高與間距，修正預覽工具列按鈕顯示。
#
# 建議安裝：
...
def suppress_customtkinter_font_warning_output() -> None:
    ...
    warnings.filterwarnings cannot catch direct stderr writes.  Keeping this
    wrapper active prevents the warning from showing in console/packaged builds,
...
class PDFRenameTool:
    ...
    def configure_root_window(self):
        ...
        width = min(1500, max(900, int(screen_w * 0.92)))
        height = min(900, max(640, int(screen_h * 0.88)))
        min_width = min(900, max(760, int(screen_w * 0.74)))
        min_height = min(650, max(560, int(screen_h * 0.68)))
    ...
    def button(self, parent, text, command, width=90, color=PRIMARY, hover=PRIMARY_HOVER,
        ...
        return ctk.CTkButton(
            ...
            command=command,
            width=width,
            height=32 if getattr(self, "_compact_layout", False) else 34,
            corner_radius=10,
            fg_color=color,
            hover_color=hover,
    ...
    def entry(self, parent, var, color="white"):
        return ctk.CTkEntry(
            parent,
            textvariable=var,
            height=28 if getattr(self, "_compact_layout", False) else 32,
            font=FONT,
            corner_radius=10,
            fg_color=color,
        )
    ...
    def apply_compact_layout(self, compact: bool):
        ...
        size = 11 if compact else UI_FONT_SIZE
        button_size = 11 if compact else 14
                ...
                try:
                    if isinstance(child, tk.Label):
                        ...
                        if isinstance(child, ctk.CTkButton):
                            child.configure(height=30 if compact else 34)
                    elif isinstance(child, (ctk.CTkEntry, ctk.CTkComboBox)):
                        child.configure(font=font, height=28 if compact else 32)
                except Exception:
                    pass
    ...
    def create_move_area(self, parent):
        ...
        move_button_row.pack(fill="x", pady=(8, 0))

        self.button(move_button_row, "??", self.browse_move_folder, 80).pack(side="left", padx=2)
        self.button(move_button_row, "???", self.move_parent_folder, 85, color=PREVIEW_BLUE, hover=PRIMARY_SOFT_HOVER, text_color="black", border=True).pack(side="left", padx=2)
        self.button(move_button_row, "????", self.undo_last_move, 100, color=YELLOW, hover=YELLOW_HOVER, text_color="black").pack(side="left", padx=2)
