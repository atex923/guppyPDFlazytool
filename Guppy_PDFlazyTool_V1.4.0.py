...
# V1.1.6   發佈整理版：更新 GitHub 首頁特色說明，並建立雲端硬碟版本資料夾同步規則。
# V1.1.7   Windows 專用：新增最小化到系統匣，可由系統匣選單顯示或結束程式。
...
class MergeTab(BaseTab):
    def __init__(self, app, notebook):
        ...
        self.files = []
        self.output_preview_images = []
        self.merge_view_mode = tk.StringVar(value="thumb")
    ...
    def _build(self):
        ...
        rounded_button(toolbar, "縮圖顯示", self.show_thumbnail_view).pack(
            side="left", padx=(10, 4)
        )
        rounded_button(toolbar, "清單顯示", self.show_list_view).pack(side="left")
        ...
        self.list_panel = ttk.Frame(list_frame, style="App.TFrame")
        self.list_panel.rowconfigure(0, weight=1)
        self.list_panel.columnconfigure(0, weight=1)
        list_body = ttk.Frame(self.list_panel, style="App.TFrame")
        ...
        list_vbar = ttk.Scrollbar(
            list_body, orient="vertical", command=self.merge_listbox.yview
        )
        list_vbar.grid(row=0, column=1, sticky="ns")
        self.merge_listbox.configure(yscrollcommand=list_vbar.set)
        list_controls = ttk.Frame(self.list_panel, style="App.TFrame")
        list_controls.grid(row=1, column=0, sticky="ew", pady=(8, 0))
        rounded_button(list_controls, "依檔名排列", self.sort_merge_files_by_name).pack(
            side="left"
        )
        rounded_button(list_controls, "上移", self.move_selected_merge_file_up).pack(
            side="left", padx=4
        )
        rounded_button(list_controls, "下移", self.move_selected_merge_file_down).pack(
            side="left"
        )
        ...
        self.enable_drop(self.list_panel, self.load_drop)
        self.enable_drop(self.merge_listbox, self.load_drop)
        self.enable_drop(output_preview, self.load_drop)

    def show_thumbnail_view(self):
        if self.merge_view_mode.get() == "thumb":
            return
        self.merge_view_mode.set("thumb")
        self.render()

    def show_list_view(self):
        if self.merge_view_mode.get() == "list":
            return
        self.merge_view_mode.set("list")
        self.render()
    ...
    def render_list(self):
        ...
        for pos, path in enumerate(self.files):
            self.merge_listbox.insert(tk.END, f"{pos + 1}. {os.path.basename(path)}")
        if selected_index is not None and self.files:
            keep = min(selected_index, len(self.files) - 1)
            self.merge_listbox.selection_set(keep)
            self.merge_listbox.see(keep)

    def selected_merge_index(self):
        if self.merge_listbox is None:
            return None
        selected = self.merge_listbox.curselection()
        if not selected:
            messagebox.showinfo("清單移動", "請先選取清單中的檔案。")
            return None
        return int(selected[0])

    def select_merge_index(self, index):
        if self.merge_listbox is None:
            return
        self.merge_listbox.selection_clear(0, tk.END)
        if 0 <= index < len(self.files):
            self.merge_listbox.selection_set(index)
            self.merge_listbox.activate(index)
            self.merge_listbox.see(index)

    def sort_merge_files_by_name(self):
        if len(self.files) < 2:
            return
        self.push_undo()
        self.files.sort(key=lambda path: os.path.basename(path).lower())
        self.clear_merge_output_preview("合併清單已依檔名排列，輸出後會重新顯示預覽。")
        self.render()
        self.select_merge_index(0)

    def move_selected_merge_file_up(self):
        index = self.selected_merge_index()
        ...
        self.render()
        self.select_merge_index(index - 1)

    def move_selected_merge_file_down(self):
        index = self.selected_merge_index()
...
class UniversalConvertPanel:
    ...
    def _build_ui(self):
        ...
        self.drop_card.grid_columnconfigure(0, weight=1)
        self.drop_card.grid_rowconfigure(0, weight=1)
        self.drop_card.grid_rowconfigure(1, weight=0)
        ...
        self.batch_stats_var = tk.StringVar(value="已拖曳/選取：0　成功：0　失敗：0")
        tk.Label(
            self.drop_card,
            textvariable=self.batch_stats_var,
            bg=CARD,
            fg=TEXT,
            font=TITLE_FONT,
            anchor="center",
    ...
    def reset_batch_stats(self, total: int):
    ...
    def update_batch_stats(self):
        self.batch_stats_var.set(
            f"已拖曳/選取：{self.batch_total}　成功：{self.batch_success}　失敗：{self.batch_failed}"
        )

    def set_batch_progress(self, success: int, failed: int):
        self.batch_success = success
    ...
    def start_conversion(self, files: list[str]):
        ...
        self.reset_batch_stats(len(valid_files))
    ...
    def _conversion_worker(self, files: list[str]):
        ...
        for file in files:
            try:
                ...
                failed.append(f"{Path(file).name}\n{exc}")
                self._write_error_log(file, exc)
            self.root.after(0, lambda s=len(success), f=len(failed): self.set_batch_progress(s, f))
        self.root.after(0, lambda: self._conversion_finished(success, failed))

    def _conversion_finished(self, success: list[Path], failed: list[str]):
        if success:
            self.last_pdf_path = success[-1]
        self.set_batch_progress(len(success), len(failed))
        self.set_busy(False)
...
class PDFRenameTool:
    ...
    def toggle_topmost_from_button(self):
        self.topmost_var.set(not bool(self.topmost_var.get()))
        self.toggle_topmost()
    ...
    def update_topmost_button(self):
        ...
        enabled = bool(self.topmost_var.get())
        button.configure(
            fg_color=PRIMARY if enabled else TAB_INACTIVE,
            text_color="white" if enabled else "black",
        )
    ...
    def create_tray_image(self):
        image_mod = Image._load()
        draw_mod = ImageDraw._load()
        image = image_mod.new("RGB", (64, 64), PRIMARY)
        ...
        draw.text((18, 16), "G", fill="white")
        draw.text((31, 31), "P", fill="white")
        return image
    ...
    def create_vertical_tabs(self):
        ...
        self.tab_bar_spacer = tk.Frame(self.tab_bar, bg=BG)
        self.tab_bar_spacer.pack(fill="both", expand=True)

        self.topmost_button = ctk.CTkButton(
            self.tab_bar,
            text="最\n上\n層",
            command=self.toggle_topmost_from_button,
            width=50,
            height=86,
            corner_radius=14,
        ...
        self.topmost_button.pack(side="bottom", pady=(0, 0), padx=8)
        self.update_topmost_button()
